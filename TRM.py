...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, dec_self_attn_mask, dec_enc_attn_mask):
        ...
        # print('dec_input.shape: ', dec_inputs.shape)
        # print('enc_outputs.shape: ', enc_outputs.shape)
        # print('dec_self_attn_mask.shape: ', dec_self_attn_mask.shape)
        # print('dec_enc_attn_mask.shape: ', dec_enc_attn_mask.shape)
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        ## dec_outputs做映射到词表大小
        dec_logits = self.projection(dec_outputs) # dec_logits : [batch_size x src_vocab_size x tgt_vocab_size] 有误？src_vocab_size应该是tgt_len
        print('dec_logits.shape : ', dec_logits.shape)
        print('dec_logits.view(-1, dec_logits.size(-1)).shape: ', dec_logits.view(-1, dec_logits.size(-1)).shape)
        # exit(0)
...
if __name__ == '__main__':
    ...
    ## 句子的输入部分，
    sentences = ['ich mochte ein bier P', 'S i want beer', 'i want beer E']
